...
let calendarEvents = {
    ...
    set mouseOverF(val) {
        this.counter = val;
        this.mouseOverListener(this);
    },
    set mouseOutF(val) {
        this.counter = val;
        this.mouseOutListener(this);
    },
    set createReservationF(val) {
        this.counter = val;
        this.createReservationListener(this);
    },

    clickListener               : function () {},
    mouseOverListener           : function () {},
    mouseOutListener            : function () {},
    createReservationListener   : function () {},

    registerClickListener               : function ( extListenerFunc ) {this.clickListener = extListenerFunc;},
    registerMouseOverListener           : function ( extListenerFunc ) {this.mouseOverListener = extListenerFunc;},
    registerMouseOutListener            : function ( extListenerFunc ) {this.mouseOutListener = extListenerFunc;},
    registerCreateReservationListener   : function ( extListenerFunc ) {this.createReservationListener  = extListenerFunc;}
}
...
export function init(params) {
    ...
    return [app.view, calendarEvents];
}
...
function onDragGridEnd() {
    if(this.dragging) {
        ...
        calendarEvents.mouse                = app.renderer.plugins.interaction.mouse.global;
        calendarEvents.mouseOver            = false;
        calendarEvents.reservationId        = id;
        calendarEvents.createReservationF   = 1;
...
function onDragRectMove(event) {
    ...
    if (this.dragging || this.dragStart) {
        ...
        this.dragging = true;
        this.dragStart = false;
        let pos = daysBetweenMonths(mouseOnDate.getTime(), firstOfMonth.getTime()) - this.hook + 2
        let offset = pos - this.startX;
        this.offset = offset
        ...
        if ((obstacles.length == 0 && 0 < mouseOnRoom && mouseOnRoom - 1 < rooms.length) ) {
            ...
            // obstacles = reservations
            // .filter((x) => (x.id !== this.parent.id))
            // .filter((x) => (x.room == this.onRoom))
            // .filter((x) => x.from.getTime() < this.parent.reservation.to.getTime()   + (timeInOneDay * offset))
            // .filter((x) => x.to.getTime()   > this.parent.reservation.from.getTime() + (timeInOneDay * offset));
            // if (obstacles.length == 0 ) {
            //     this.parent.y = (this.onRoom - this.parent.room) * squareSize;
            //     this.parent.x = pos*squareSize
            //     this.from = new Date(this.parent.reservation.from.getTime() + this.offset*timeInOneDay)
            //     this.to = new Date(this.parent.reservation.to.getTime() + this.offset*timeInOneDay)
            // }
    ...
    app.ticker.add((delta) => {
        ...
        if (mouseX > 0 && mouseY > 0 && mouseX < screenWidth && mouseY < screenHeight) {
            ...
            if (nRooms*squareSize > screenHeight) {
                ...
                if  (mouseY > screenHeight - 100 && posY > screenHeight - nRooms*squareSize - squareSize) {
                    verticalVelocity = 5;
                    // console.log(posY)
                } else if (mouseY < 100 && posY < squareSize*2) {
                    verticalVelocity = -5;
                } else {
                    verticalVelocity = 0;
                }
